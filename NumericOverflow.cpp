...
{
    //GM: Initialize numeric_limits of max and min from template
    ...
    { //GM: If the current result will create an overflow when incremented, raise an error
       
            if (result >= 0 && increment > max - result) {
                throw std::overflow_error("An Overflow error occured.");
            }
            //GM: If there is no error, increment as normal
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
         //GM: If the current result will create an underflow when decremented, raise an error

        if (result < 0 && decrement < min - result) {
            throw std::underflow_error("An underflow error occured.");

        }
        //GM: If there is no error, decrement as normal.
        result -= decrement;
    }
...
void test_overflow()
{
    ...
    //GM: Wrapping each statement in a try to test for issues while executing
    try {
    ...
    //GM: Catch statement in case an error is found. If so, print what the error is
