...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //GM: Storing SQL query beginnning and end into one variable and
  //Adding a regex pattern checker to check for unwanted input,
  //Then compare input with checker and exit if attack found.
  std::string sqlQuery(sql);
  std::regex pattern("('(.*)[a-zA-Z](.*)'='(.*)[a-zA-Z](.*)')|([0-9]=[0-9])");
  std::smatch match;

  if (std::regex_search(sqlQuery, match, pattern) && match.size() > 1) {
      std::cout << "SQL Injection found!" << std::endl;
      return false;
  }
